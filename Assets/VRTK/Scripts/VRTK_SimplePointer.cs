...
namespace VRTK
{
    ...
    public class VRTK_SimplePointer : VRTK_WorldPointer
    {
        ...
        //private VRTK_ObjectTooltip targetTooltip;
        ...
        private float GetPointerBeamLength(bool hasRayHit, RaycastHit collidedWith)
        {
            ...
            if (hasRayHit)
            {
              //  VRTK_ObjectTooltip[] tooltips = FindObjectsOfType(typeof(VRTK_ObjectTooltip)) as VRTK_ObjectTooltip[];
                //foreach (VRTK_ObjectTooltip tooltip in tooltips) tooltip.enabled = false;

                pointerContactDistance = collidedWith.distance;
                pointerContactTarget = collidedWith.transform;
                destinationPosition = pointerTip.transform.position;

                //   targetTooltip =  pointerContactTarget.GetComponent<VRTK_ObjectTooltip>();
                // if(targetTooltip != null) targetTooltip.enabled = true;

                if (hudtext)
                {
                    hudtext.text = collidedWith.transform.name;
                }
